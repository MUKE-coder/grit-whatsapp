...
// StartImport records a CSV import of participants about to run: the job a client
// polls for progress.
func (s *ParticipantService) StartImport(ctx context.Context, total int) (*models.ImportJob, error) {
	// CreatedBy is who may follow the job: GET /imports/:id answers only them,
	// or an admin.
	job := models.ImportJob{Resource: "participants", Status: "processing", Total: total, CreatedBy: authz.UserIDFrom(ctx)}
	if err := s.db(ctx).Create(&job).Error; err != nil {
		return nil, fmt.Errorf("starting the participants import: %w", err)
	}
	return &job, nil
}

// ImportCSV streams the CSV at path, creating participants in batches and
...
func (s *ParticipantService) ImportCSV(ctx context.Context, jobID, path string) {
	db := s.db(ctx)
	...
	flush := func() {
		...
		items := make([]models.Participant, len(batch))
		for i := range batch {
	...
	for {
		...
		if v, ok := get(rec, "muted"); ok {
			item.Muted = v == "true" || v == "1" || v == "yes"
		}
