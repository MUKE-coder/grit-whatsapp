...
// ParticipantService owns every database read and write for participants.
//
...
// participantListConfig is what a client may search, sort and filter participants
// by. Whitelisted, because each name ends up in SQL.
var participantListConfig = paginate.Config{
	Searchable: []string{},
	Sortable:   map[string]bool{"id": true, "created_at": true, "role": true},
	Filterable: map[string]bool{"id": true, "conversation_id": true, "user_id": true, "role": true, "last_read_at": true, "last_delivered_at": true, "muted": true},
}

// writableParticipant is every column Patch and Bulk may write. id, the
// timestamps and the version are the framework's, and are dropped.
var writableParticipant = map[string]bool{
...
func (s *ParticipantService) db(ctx context.Context) *gorm.DB {
...
func (s *ParticipantService) write(db *gorm.DB) *gorm.DB {
...
func (s *ParticipantService) returning(db *gorm.DB) bool {
...
func (s *ParticipantService) relations(db *gorm.DB, item *models.Participant) error {
...
// List returns one page of participants.
//
//	archived "true" or "1"   only archived rows
//	archived "all"           both
//	anything else            only live rows
func (s *ParticipantService) List(ctx context.Context, p paginate.Params, archived string) (paginate.Result[models.Participant], error) {
	query := s.db(ctx).Model(&models.Participant{}).Preload("Conversation").Preload("User")
	...
	return paginate.List[models.Participant](query, p, participantListConfig)
}

// Export hands every matching participant to each, a batch at a time, so a large
...
func (s *ParticipantService) Export(ctx context.Context, search string, each func(rows []models.Participant) error) error {
	query := s.db(ctx).Model(&models.Participant{}).Preload("Conversation").Preload("User")
	if search != "" {
		clause := ""
		args := []any{}
		wild := "%" + search + "%"
		for i, col := range participantListConfig.Searchable {
			if i > 0 {
				clause += " OR "
			}
	...
	var rows []models.Participant
	return query.FindInBatches(&rows, 1000, func(tx *gorm.DB, batch int) error {
		return each(rows)
	}).Error
}

// GetByID returns one participant with the relations shown beside it.
func (s *ParticipantService) GetByID(ctx context.Context, id string) (*models.Participant, error) {
	var item models.Participant
	if err := s.db(ctx).Preload("Conversation").Preload("User").First(&item, "id = ?", id).Error; err != nil {
		return nil, err
	}
	return &item, nil
}
...
func (s *ParticipantService) load(ctx context.Context, id string) (*models.Participant, error) {
	var item models.Participant
...
// the row is at now.
func (s *ParticipantService) conflict(ctx context.Context, id string) error {
...
// Create saves a new participant and fills item in as it was stored.
func (s *ParticipantService) Create(ctx context.Context, item *models.Participant) error {
	db := s.db(ctx)
	...
	} else if err := db.Preload("Conversation").Preload("User").First(item, "id = ?", item.ID).Error; err != nil {
		return err
	}
	return nil
}

// Update writes updates to one participant. With a precondition it lands only
// if the row is still at that version, and otherwise returns an
// *concurrency.ErrConflict naming the version it is at.
func (s *ParticipantService) Update(ctx context.Context, id string, updates map[string]interface{}, pre *concurrency.Precondition) (*models.Participant, error) {
...
func (s *ParticipantService) Patch(ctx context.Context, id string, body map[string]interface{}, pre *concurrency.Precondition) (*models.Participant, map[string]interface{}, error) {
	item, err := s.load(ctx, id)
	if err != nil {
		return nil, nil, err
	}
...
// Delete soft-deletes one participant and returns it as it was.
func (s *ParticipantService) Delete(ctx context.Context, id string) (*models.Participant, error) {
	item, err := s.load(ctx, id)
	if err != nil {
		return nil, err
	}
...
// ParticipantBulkResult is what a bulk action did.
type ParticipantBulkResult struct {
...
// Bulk applies one action to many participants in a single transaction: all of
// it lands or none of it does. action is delete, archive, restore or patch.
func (s *ParticipantService) Bulk(ctx context.Context, action string, ids []string, patch map[string]interface{}) (ParticipantBulkResult, error) {
	db := s.db(ctx)
	var result ParticipantBulkResult

	// Unarchived rows for archive, archived for restore: without it a mixed
	// selection reports "12 archived" having changed three.
	scope := db.Model(&models.Participant{}).Where("id IN ?", ids)
	if action == "restore" {
	...
	if err := scope.Find(&items).Error; err != nil {
		return result, fmt.Errorf("loading participants: %w", err)
	}
	...
	if action == "patch" {
		...
		if len(result.Updates) == 0 {
			return ParticipantBulkResult{}, respond.Rule("No writable fields in patch")
		}
	}
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		switch action {
		case "delete":
			return tx.Where("id IN ?", result.IDs).Delete(&models.Participant{}).Error
		case "archive":
			return tx.Model(&models.Participant{}).Where("id IN ?", result.IDs).
	...
	if err != nil {
		return ParticipantBulkResult{}, err
	}
	return result, nil
}
