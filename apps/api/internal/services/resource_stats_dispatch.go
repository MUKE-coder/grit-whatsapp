...
func ComputeResourceStats(db *gorm.DB, resourceName string, filter ResourceStatsFilter) (*ResourceStats, error) {
	...
	case "conversations":
		return reflectiveResourceStats(db, resourceName, &models.Conversation{}, filter)

	case "participants":
		return reflectiveResourceStats(db, resourceName, &models.Participant{}, filter)

	case "messages":
		return reflectiveResourceStats(db, resourceName, &models.Message{}, filter)
...
// buildDailySeries returns 30 buckets, one per UTC calendar day for the last 30
