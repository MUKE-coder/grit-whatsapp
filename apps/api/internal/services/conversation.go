...
// ConversationService owns every database read and write for conversations.
//
...
// conversationListConfig is what a client may search, sort and filter conversations
// by. Whitelisted, because each name ends up in SQL.
var conversationListConfig = paginate.Config{
	Searchable: []string{"title", "last_message_preview"},
	Sortable:   map[string]bool{"id": true, "created_at": true, "title": true, "last_message_preview": true},
	Filterable: map[string]bool{"id": true, "title": true, "is_group": true, "last_message_at": true, "last_message_preview": true},
}

// writableConversation is every column Patch and Bulk may write. id, the
// timestamps and the version are the framework's, and are dropped.
var writableConversation = map[string]bool{
	"title":                true,
	"is_group":             true,
...
func (s *ConversationService) db(ctx context.Context) *gorm.DB {
...
func (s *ConversationService) write(db *gorm.DB) *gorm.DB {
...
func (s *ConversationService) returning(db *gorm.DB) bool {
...
// List returns one page of conversations.
//
//	archived "true" or "1"   only archived rows
//	archived "all"           both
//	anything else            only live rows
func (s *ConversationService) List(ctx context.Context, p paginate.Params, archived string) (paginate.Result[models.Conversation], error) {
	query := s.db(ctx).Model(&models.Conversation{})
	...
	return paginate.List[models.Conversation](query, p, conversationListConfig)
}

// Export hands every matching conversation to each, a batch at a time, so a large
...
func (s *ConversationService) Export(ctx context.Context, search string, each func(rows []models.Conversation) error) error {
	query := s.db(ctx).Model(&models.Conversation{})
	if search != "" {
		clause := ""
		args := []any{}
		wild := "%" + search + "%"
		for i, col := range conversationListConfig.Searchable {
			if i > 0 {
				clause += " OR "
			}
...
// GetByID returns one conversation with the relations shown beside it.
...
func (s *ConversationService) load(ctx context.Context, id string) (*models.Conversation, error) {
	var item models.Conversation
	if err := s.db(ctx).First(&item, "id = ?", id).Error; err != nil {
		return nil, err
	}
...
// the row is at now.
func (s *ConversationService) conflict(ctx context.Context, id string) error {
...
// Create saves a new conversation and fills item in as it was stored.
func (s *ConversationService) Create(ctx context.Context, item *models.Conversation) error {
	db := s.db(ctx)
...
// Update writes updates to one conversation. With a precondition it lands only
// if the row is still at that version, and otherwise returns an
// *concurrency.ErrConflict naming the version it is at.
func (s *ConversationService) Update(ctx context.Context, id string, updates map[string]interface{}, pre *concurrency.Precondition) (*models.Conversation, error) {
...
func (s *ConversationService) Patch(ctx context.Context, id string, body map[string]interface{}, pre *concurrency.Precondition) (*models.Conversation, map[string]interface{}, error) {
	item, err := s.load(ctx, id)
	if err != nil {
		return nil, nil, err
	}
...
// Delete soft-deletes one conversation and returns it as it was.
func (s *ConversationService) Delete(ctx context.Context, id string) (*models.Conversation, error) {
	item, err := s.load(ctx, id)
	if err != nil {
		return nil, err
	}
...
// ConversationBulkResult is what a bulk action did.
...
// Bulk applies one action to many conversations in a single transaction: all of
// it lands or none of it does. action is delete, archive, restore or patch.
func (s *ConversationService) Bulk(ctx context.Context, action string, ids []string, patch map[string]interface{}) (ConversationBulkResult, error) {
	db := s.db(ctx)
	var result ConversationBulkResult

	// Unarchived rows for archive, archived for restore: without it a mixed
	// selection reports "12 archived" having changed three.
	scope := db.Model(&models.Conversation{}).Where("id IN ?", ids)
	if action == "restore" {
	...
	if err := scope.Find(&items).Error; err != nil {
		return result, fmt.Errorf("loading conversations: %w", err)
	}
	...
	if action == "patch" {
		...
		if len(result.Updates) == 0 {
			return ConversationBulkResult{}, respond.Rule("No writable fields in patch")
		}
	}
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		switch action {
		case "delete":
			return tx.Where("id IN ?", result.IDs).Delete(&models.Conversation{}).Error
		case "archive":
			return tx.Model(&models.Conversation{}).Where("id IN ?", result.IDs).
	...
	if err != nil {
		return ConversationBulkResult{}, err
	}
	return result, nil
}
