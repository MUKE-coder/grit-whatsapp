...
// Import kicks off a BACKGROUND CSV import of messages. It streams the upload
...
func (h *MessageHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsMessage(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
...
// countCSVRowsMessage counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsMessage(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// Template returns a ready-to-fill CSV template (header row) for importing messages.
...
func (h *MessageHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="messages-template.csv"`)
	c.String(http.StatusOK, "conversation,sender,body,kind\n")
}
