...
// Import kicks off a BACKGROUND CSV import of participants. It streams the upload
...
func (h *ParticipantHandler) Import(c *gin.Context) {
	...
	total, err := countCSVRowsParticipant(tmpPath)
	if err != nil || total < 0 {
		os.Remove(tmpPath)
...
// countCSVRowsParticipant counts data rows (excluding the header) without holding
// the file in memory.
func countCSVRowsParticipant(path string) (int, error) {
	f, err := os.Open(path)
	if err != nil {
		return -1, err
	}
...
// Template returns a ready-to-fill CSV template (header row) for importing participants.
...
func (h *ParticipantHandler) Template(c *gin.Context) {
	c.Header("Content-Type", "text/csv")
	c.Header("Content-Disposition", `attachment; filename="participants-template.csv"`)
	c.String(http.StatusOK, "conversation,user,role,muted\n")
}
