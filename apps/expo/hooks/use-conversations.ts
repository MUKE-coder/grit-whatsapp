...
// foreign key) to scope the list: useConversations("", { category_id: id }).
export function useConversations(
  ...
  return useInfiniteQuery({
    queryKey: ["conversations", { search, filters, sortBy, sortOrder, pageSize }],
    initialPageParam: 1,
    queryFn: async ({ pageParam }) => {
      ...
      return (await api.get("/conversations?" + qs.toString())) as ConversationsPage;
    },
...
export function useConversation(id: string) {
  return useQuery<Conversation>({
    ...
    queryFn: async () => (await api.get("/conversations/" + id)).data,
    enabled: !!id,
  });
}
...
export function useCreateConversation() {
  ...
  return useMutation({
    mutationFn: async (input: Record<string, unknown>) => api.post("/conversations", input),
...
export function useUpdateConversation() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...input }: { id: string } & Record<string, unknown>) =>
      api.put("/conversations/" + id, input),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["conversations"] }),
  });
}
...
export function useDeleteConversation() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (id: string) => api.delete("/conversations/" + id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["conversations"] }),
  });
}
