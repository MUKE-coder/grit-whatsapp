...
function PaneHeader({
  ...
  return (
    <header className="flex items-center gap-3 border-border border-b bg-bg-secondary px-4 py-2">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p
          className={cn("truncate text-xs", typists.length > 0 ? "text-success" : "text-text-secondary")}
          aria-live="polite"
        >
          {status}
        </p>
...
function MessageList({ conv, me }: { conv: Conversation; me: Me }) {
  ...
  return (
    <div
      ref={scroller}
      onScroll={onScroll}
      className="min-h-0 flex-1 overflow-y-auto bg-bg-tertiary px-4 py-3 md:px-10"
      role="log"
      aria-label="Messages"
    >
      {isFetchingNextPage && <p className="py-2 text-center text-text-muted text-xs">Loading earlier messages…</p>}
      {isLoading && <p className="py-10 text-center text-sm text-text-muted">Loading messages…</p>}
      {!isLoading && messages.length === 0 && (
        <p className="mx-auto my-10 max-w-xs rounded-lg bg-bg-elevated px-4 py-3 text-center text-sm text-text-secondary">
        ...
        {messages.map((m, i) => {
          ...
          return (
            <Fragment key={m.client_id ?? m.id}>
              {newDay && (
                <li className="my-3 self-center rounded-md bg-bg-elevated px-3 py-1 text-text-secondary text-xs shadow-sm">
    ...
    <li className={cn("flex", mine ? "justify-end" : "justify-start", spaced && "mt-2")}>
      <div
        className={cn(
          ...
          mine ? "bg-accent text-white" : "bg-bg-elevated text-foreground",
        )}
      >
...
function Composer({ conversationId, me }: { conversationId: string; me: Me }) {
  ...
  return (
    <form
      className="flex items-end gap-2 border-border border-t bg-bg-secondary px-3 py-2"
      onSubmit={(e) => {
        e.preventDefault();
        submit();
      }}
    >
      ...
      <label className="flex-1">
        ...
        <textarea
          ...
          className="max-h-40 w-full resize-none rounded-lg bg-background px-3 py-2 text-foreground text-sm outline-none placeholder:text-text-muted"
        />
      </label>
      <button
        type="submit"
...
function Empty({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex h-full items-center justify-center bg-bg-tertiary text-sm text-text-muted">{children}</div>
  );
}
