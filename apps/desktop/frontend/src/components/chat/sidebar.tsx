...
export function Sidebar({
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col border-border border-r bg-surface">
      {view === "inbox" && (
  ...
  return (
    <>
      ...
      <ul className="min-h-0 flex-1 overflow-y-auto">
        {isLoading && <li className="px-4 py-6 text-sm text-foreground-muted">Loading chats…</li>}
      ...
      >
        ...
        <span className="min-w-0 flex-1 border-border border-b pb-3">
          <span className="flex items-baseline gap-2">
            ...
            <span className={cn("shrink-0 text-xs", conv.unread > 0 ? "text-success" : "text-foreground-muted")}>
          ...
          <span className="mt-0.5 flex items-center gap-2">
            ...
            {conv.muted && <BellOff className="h-4 w-4 shrink-0 text-foreground-muted" aria-label="Muted" />}
            {conv.unread > 0 && (
...
function NewChat({ onBack, onStarted }: { onBack: () => void; onStarted: (id: string) => void }) {
  ...
  return (
    <>
      ...
      {start.isError && (
        <p className="px-4 py-2 text-danger text-sm">{errorMessage(start.error, "Could not start that chat.")}</p>
      )}
      <PeopleList
        users={users.data}
    ...
    <>
      ...
      >
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-foreground-secondary">Group name</span>
          <input
            value={title}
            ...
            {picked.map((u) => (
              <li key={u.id}>
                <button
                  type="button"
                  onClick={() => toggle(u)}
                  className="rounded-full bg-surface-2 px-3 py-1 text-foreground text-sm hover:bg-surface-hover"
                >
        ...
        {create.isError && (
          <p className="text-danger text-sm">{errorMessage(create.error, "Could not create the group.")}</p>
        )}
        <button
          type="submit"
...
function PeopleList({
  ...
  if (loading) return <p className="px-4 py-6 text-sm text-foreground-muted">Searching…</p>;
  if (!users || users.length === 0) return <p className="px-4 py-6 text-sm text-foreground-muted">Nobody found.</p>;
  return (
    <ul className="min-h-0 flex-1 overflow-y-auto">
      {users.map((u) => (
        <li key={u.id}>
          <button
            ...
            className={cn(
              ...
              selected?.has(u.id) && "bg-surface-2",
            )}
          >
...
function SearchBox({
  ...
  return (
    <div className="px-3 py-2">
      <label className="flex items-center gap-2 rounded-lg bg-surface-2 px-3 py-2">
        ...
        <input
          ...
          autoFocus={autoFocus}
          className="w-full bg-transparent text-foreground text-sm outline-none placeholder:text-foreground-muted"
        />
      </label>
    </div>
  );
...
export function IconButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className="rounded-full p-2 text-foreground-secondary hover:bg-surface-hover hover:text-foreground"
    >
      {children}
    </button>
  );
