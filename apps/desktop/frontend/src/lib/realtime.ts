...
// The access token the desktop app keeps in the OS keychain, read on every
 ...
 *
 * A desktop webview's cookies do not reach the API, so the socket passes the token itself. By
...
export async function connect(): Promise<void> {
  ...
  const ws = new WebSocket(WS_URL + await authQuery());
  socket = ws;
